...
$nomeAssunto = @$_POST['nomeAssunto'];



if ($nomePesquisa == "" and $nomeAssunto == "") {



$query = $pdo->query("SELECT * from $tabela where data_fechamento >= '$dataInicial' and data_fechamento <= '$dataFinal 23:59:59' and status = 'F' order by data_fechamento desc");
...
<small>
    ...
    <th class="esc">Pontos</th>
    <th class="esc">Pontos_negativo</th>
...
HTML;
    ...
    $query = $pdo->query("
        ...
        and funcionario LIKE '%$nomePesquisa%'
    ");
    ...
    $query = $pdo->query("SELECT * from $tabela where data_fechamento >= '$dataInicial' and data_fechamento <= '$dataFinal 23:59:59' and funcionario LIKE '%$nomePesquisa%' and assunto LIKE '%$nomeAssunto%' order by data_fechamento desc");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$linhas = @count($res);
if($linhas > 0){
echo <<<HTML
...
echo '<div style="text-align: center; font-size: 20px;">Total de Pontos: ' . $totalPontos . '</div>';
